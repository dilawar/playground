...
class DCSettings:
      ...
      if node.nodeType == node.TEXT_NODE:
        ...
        for dirNode in node.childNodes:
          if dirNode.nodeName == "dir":
            ...
            print "Parse error at share.dir"
            raise ParseError(ParseError.E_SYNTAX)
        ...
        for sourceNode in node.childNodes:
          if sourceNode.nodeName == "source":
            ...
            if i[0] != 'http' or len(i[1]) == 0:
              print "Source entry in setting is either empty of missing http prefix."
            ...
            print "Parse error at hublist.source"
            print node.nodeType
            raise ParseError(ParseError.E_SYNTAX)
        ...
        print "Parse error at sharesize"
        raise ParseError(ParseError.E_SYNTAX)
  ...
  def loadValue(self, node, required = 1):
    if len(node.childNodes) != 1:
      if required:
        print "Parse error : required entry missing."
        raise ParseError(ParseError.E_SYNTAX)
      else:
        return None
    ...
    if valueNode.nodeType != node.TEXT_NODE:
      print "Parse error at childnode."
      raise ParseError(ParseError.E_SYNTAX)
